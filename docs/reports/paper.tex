...
\caption{Accuracy, macro F1, and weighted F1 for all five LoRA-adapted backbones on the Bhagavata Purana Sanskrit PoS test set, sorted by macro F1.}
\label{fig:benchmark_comparison}
\end{figure}

Figure~\ref{fig:benchmark_comparison} visualizes the summary in Table~\ref{tab:benchmark_summary}. The gap between accuracy and macro F1 is largest for \texttt{xlm-roberta-base}: its accuracy (0.6894) looks moderate, but its macro F1 collapses to 0.1632 because the model collapses to predicting NOUN for nearly every token, as the per-class breakdown in Section~\ref{sec:perclass} confirms. This is exactly the failure mode that motivates our choice of macro F1, rather than accuracy, as the primary ranking metric.
...
\subsection{Per-Class Performance Across Models}
\label{sec:perclass}
Table~\ref{tab:perclass_f1} reports the per-class F1 score of every model on each of the five POS labels, and Fig.~\ref{fig:perclass_f1} visualizes the same values. The highest F1 in each row is shown in bold.

\begin{table*}[htbp]
\caption{Per-Class F1 Scores Across Models on the Bhagavata Purana PoS Test Set}
\begin{center}
\begin{tabular}{|l|c|c|c|c|c|}
\hline
\textbf{Class} & \textbf{IndicBERT} & \textbf{MuRIL} & \textbf{XLM-R} & \textbf{Hindi-BERT-v2} & \textbf{IndicBERTv2} \\
\hline
IND  & 0.7514 & 0.9204 & 0.0000 & 0.9218 & \textbf{0.9525} \\
\hline
NOUN & 0.8790 & 0.9675 & 0.8161 & 0.9669 & \textbf{0.9803} \\
\hline
PART & 0.6943 & 0.9503 & 0.0000 & 0.9623 & \textbf{0.9809} \\
\hline
PRON & 0.8301 & 0.9673 & 0.0000 & \textbf{0.9767} & 0.9789 \\
\hline
VERB & 0.5737 & 0.8655 & 0.0000 & 0.8586 & \textbf{0.9110} \\
\hline
\end{tabular}
...
This study has several limitations. First, the corpus remains imbalanced across the five POS classes. This imbalance can still affect generalization, especially for PART, PRON, and VERB. Second, the benchmark focuses on validation performance within one project dataset; it does not yet test cross-corpus transfer or broader domain robustness. Third, the current system still depends on rule-based preprocessing and analyzer-driven label selection. These choices improve quality, but they also tie the pipeline to the current annotation logic. Fourth, we report per-class F1 (Table~\ref{tab:perclass_f1}) but not a full token-level confusion matrix, since confusion counts were not logged during the training runs reported here; consequently, we cannot yet identify which specific classes \texttt{xlm-roberta-base} confuses NOUN with, only that it does. Fifth, we did not run a component-wise ablation isolating the individual contribution of data cleaning, sandhi splitting, and rule-based correction; all five benchmarked models share the same fully preprocessed corpus.
